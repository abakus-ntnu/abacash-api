...
const parseSystemRoles = () =>
  db
    .many('SELECT * FROM "systemRoles"')
    .then(customers =>
      customers.map(customer => ({ model: 'SystemRole', data: customer }))
    )
    .then(customers =>
      jsonfile.writeFileAsync(`${temp}/system-roles.json`, customers)
    );

const parseCustomer = () =>
  db
    .many('SELECT * FROM customers')
    .then(customers =>
      customers.map(customer => ({ model: 'Customer', data: customer }))
    )
    .then(customers =>
      jsonfile.writeFileAsync(`${temp}/customers.json`, customers)
    );
...
const parseTransactions = () =>
  db
    .many('SELECT * FROM transactions')
    .then(trans =>
      trans.map(transaction => ({ model: 'Transaction', data: transaction }))
    )
    .then(trans => jsonfile.writeFileAsync(`${temp}/transactions.json`, trans));

const parseCustomerRoles = () =>
  db
    .many('SELECT * FROM "customerRoles"')
    .then(roles => roles.map(role => ({ model: 'CustomerRole', data: role })))
    .then(roles =>
      jsonfile.writeFileAsync(`${temp}/customer-roles.json`, roles)
    );

const parseAPITokens = () =>
  db
    .many('SELECT * FROM "APITokens"')
    .then(tokens => tokens.map(token => ({ model: 'APIToken', data: token })))
    .then(tokens => jsonfile.writeFileAsync(`${temp}/api-tokens.json`, tokens));

const parseTransactionProducts = () =>
  db
    .many('SELECT * FROM "transactionProducts"')
    .then(trans =>
      trans.map(tran => ({ model: 'TransactionProduct', data: tran }))
    )
    .then(trans =>
      jsonfile.writeFileAsync(`${temp}/transaction-products.json`, trans)
    );

const parseSystems = () =>
  db
    .many('SELECT * FROM systems')
    .map(systemData => {
        const system = {
            model: 'System',
            data: omit(systemData, ['productTypes'])
        };
        const productGroups = systemData.productTypes.map(type => ({
            model: 'ProductGroup',
            data: { name: type, systemId: systemData.id }
        }));
    ...
    .then(data => {
        ...
        productGroups = productGroups.map((group, id) => ({
            model: 'ProductGroup',
            data: { ...group.data, id: id + 1 }
        }));
        return Promise.all([
