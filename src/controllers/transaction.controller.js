...
export function add(req, res, next) {
  ...
  db.sequelize
    .transaction(t => {
      ...
      if (config.needSeller && !req.body.sellerId) {
        return next(
          new errors.ValidationError(
      ...
      return (
        checkIfSellerIsSeller(req.body.sellerId, config.needSeller)
          .then(isSeller => {
            if (!isSeller) {
              throw new errors.ValidationError(
                'sellerId does not belong to a seller'
              );
            }
          ...
          .then(customer => {
            if (!customer) {
              throw new errors.ValidationError(
                'Customer for transaction not found'
              );
            }
          ...
          .then(customerRole => {
            ...
            return Bluebird.mapSeries(req.body.products, id =>
              db.Product.findById(id, { transaction: t }).then(product => {
                if (!product) {
                  throw new errors.ValidationError(
          ...
          .reduce(
            (sum, product) =>
              (currentCustomerRole.internalSales
          ...
          .then(transaction => {
            ...
            if (
              ...
              currentCustomerRole.allowCredit !== true
            ) {
    ...
    .catch(
      err => err.parent && err.parent.code === SERIALIZATION_FAILURE,
      err => {
        errors.handleError(err);
