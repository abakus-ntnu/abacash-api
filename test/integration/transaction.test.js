  ...
  return new Bluebird((resolve, reject) => {
    request(app)
      ...
      .end((err, res) => {
        ...
        res.body.products.length.should.equal(
          Object.keys(countBy(products)).length
        );
        resolve();
      });
  });
...
describe('Transaction API Integration', () => {
  const fixtures = ['api-tokens.json', 'integration/transaction.json'];
  ...
  it('should allow a user with allowCredit role to get negative balance', () => {
    ...
    // add the transaction
    return postTransactionAndCheckSum(body, 105.0)
      .then(transaction => checkIfTransactionExists(transaction, body.products))
  ...
  it('should not decrease stock if keepStock is false', () => {
    ...
    return postTransactionAndCheckSum(body, 10.0).then(() =>
      checkProductStock(3, 8)
    );
  });
